...
def predict(**params):
    ...
    #编译 优化器使用的是Adam 损失函数是广义的dc值 检测dc值
    ...
    for file in files_test:
        ...
        pred_prob = pred[0]
        # 找到沿第3个维度返回的最大值的索引 其实也就是为true的值对应不同的类别
        ...
        # pred之前已经获取到对应的类别坐标，ind对应与不同的几个类别，并将其置为对应的颜色
        ...
        # for ind in range(len(color_code_labels)):
        #     label_pred[pred == ind, :] = color_code_labels[ind]
