...
def create_datasets(**params):
    ...
    for file in files_train:
        ...
        for lab in range(len(labels)):
            try:
                image = imread(gt_path + labels[lab] + img_name + '_' + labels[lab][:-1] + '.tif',as_gray=True)
            ...
            # print((label[:, :] == 1))
            # print(image_all_labels.sum())
            # image_all_labels[(label[:, :] == 1)] = (((lab+1) / 4) * 255)
            image_all_labels[(label[:, :] == 1)] = (lab + 1)
            # print(image_all_labels.sum())
    ...
    for file in files:
        ...
        for lab in range(len(labels)):
            try:
                ...
                flag = False
            if flag:
                image_all_labels[(label[:,:]== 1)] = color_code_labels[lab+1]
            else:
                image_all_labels[(label[:, :,0] == 1)] = color_code_labels[lab + 1]
